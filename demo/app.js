...
const viewer = new Viewer({ canvasId: "cv", transparent: false });
...
$("gen").onclick = async () => {
  ...
  try {
    ...
    loadScene(d.glb, d.metamodel);
    renderTable(d.items);
  } catch (e) { setMsg("Request failed: " + e, true); }
};

function loadScene(glb, meta) {
  if (model) model.destroy();
  const bust = "?t=" + Date.now();
  model = gltf.load({ id: "room", src: glb + bust, metaModelSrc: meta + bust, edges: true });
  model.on("loaded", () => viewer.cameraFlight.jumpTo(model));
  model.on("error", (e) => setMsg("3D load error: " + e, true));
}
