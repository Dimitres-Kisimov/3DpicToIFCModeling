...
| **Branch under analysis** | `retrieval-pivot-blueprint` (HEAD `c78f3ac`) |
| **Reporting period** | Phase 1 setup through model survey and pivot consolidation |
| **Report date** | 6 June 2026 |
| | |
| **Submitted by** | [Author Name] |
| **Identifier / Matrikel-Nr.** | [ID — to be completed by author] |
| **Role** | Project Engineer |
| **Submitted to** | SCS — Technical and Management Review |
| | |
| **Hardware platform** | NVIDIA GeForce RTX 4070 Laptop GPU (8 GB VRAM), 64 GB system RAM, Windows 11 Pro |
| **Software baseline** | Node.js 24.15.0, Python 3.13.3, PyTorch 2.12.0+cu126, transformers 5.10.2 |
| **Document type** | Technical Report |
| **Distribution** | Internal — for SCS technical and management review |
| **Version** | 1.0 |
...
|---|---|
| ABO | Amazon Berkeley Objects (dataset) |
| ADE20K | MIT Scene-Parsing dataset (150 classes) |
| AGPL | GNU Affero General Public License |
| API | Application Programming Interface |
| BIM | Building Information Modeling |
| BSD | Berkeley Software Distribution (licence family) |
| CC | Creative Commons (licence family) |
...
| CUDA | Compute Unified Device Architecture (NVIDIA GPU runtime) |
| DETR | DEtection TRansformer |
| DINO | DIstillation with NO labels (self-supervised vision transformer) |
| FAISS | Facebook AI Similarity Search |
| FP16 | 16-bit floating-point precision |
| FP32 | 32-bit floating-point precision |
| GLB | binary glTF 3D mesh format |
| GPU | Graphics Processing Unit |
| HF | Hugging Face (model and dataset host) |
...
| MIT | Massachusetts Institute of Technology (licence family) |
| NMS | Non-Maximum Suppression |
| NVMe | Non-Volatile Memory Express (SSD interface) |
| OOM | Out-Of-Memory error |
| OV | Open-Vocabulary (detection paradigm) |
| OWL-ViT | Open-World Localisation Vision Transformer |
| PBR | Physically-Based Rendering (material parameter set) |
| ResNet | Residual Network (image-classification backbone) |
| RT-DETR | Real-Time DEtection TRansformer |
| SAM | Segment Anything Model |
| SCS | The project sponsor (employer of the author) |
| SDK | Software Development Kit |
| SF3D | Stable Fast 3D |
| SigLIP | Sigmoid-Loss Image-Language Pre-training |
| SLAT | Structured 3D Latents (TRELLIS representation) |
| SLA | Service Level Agreement |
| TRELLIS | Structured 3D Latents for Scalable and Versatile 3D Generation |
| TripoSR | Tripo single-view reconstruction model |
| URL | Uniform Resource Locator |
| UV | UV mapping (texture-coordinate space) |
| ViT | Vision Transformer |
| VRAM | Video Random-Access Memory (GPU memory) |
| XKT | xeokit's binary scene-graph format |
| YOLO | You Only Look Once (single-shot detector) |
...
|---|---|---|
| Figure 1 | High-level architecture of the recommended retrieval pipeline (input → detection → segmentation → retrieval → scale and export) | §5.1 |
| Figure 2 | Repository branch graph reproducing the project's nine-branch history with line markers A–E | Appendix C |
...
|---|---|---|
| Table 1 | CLIP fine-tune training corpus: per-category image counts across the eleven SCS office categories | §4.5 |
| Table 2 | Nine project branches with last-commit hash, date, and subject | §4.7 |
| Table 3 | Cross-pipeline model survey — ten candidate models spanning retrieval, segmentation, depth, detection, and generation stages | §6.2 |
| Table 4 | Detection-specific model survey — ten candidate detectors and classifiers evaluated for office-furniture recognition | §6.3 |
| Table 5 | Workstation specification as tested on the SCS engineer's laptop | §7.5.1 |
| Table 6 | Per-stage binding hardware constraint and headroom analysis | §7.5.2 |
| Table 7 | Recommended hardware Scenario A — single-user developer workstation | §7.5.4 |
| Table 8 | Recommended hardware Scenario B — single-machine deployment with high-fidelity fallback | §7.5.4 |
| Table 9 | Recommended hardware Scenario C — multi-user batch back-office at scale | §7.5.4 |
| Table 10 | Per-stage wall-clock latency estimates for capacity planning | §7.5.5 |
| Table 11 | Sprint-level roadmap for the recommended Apache-2.0 retrieval primary path | §11.2 |
| Table 12 | Risk register with probability, impact, and mitigation per identified risk | §11.3 |
| Table 13 | Verbatim licence string verification table for every model considered | Appendix A |
| Table 14 | System-level prerequisites with installed versions and roles | §17.1 |
| Table 15 | Node.js direct dependencies | §17.2 |
| Table 16 | Python deep-learning runtime packages with installed versions | §17.3.1 |
| Table 17 | Python machine-learning and HuggingFace stack packages | §17.3.2 |
| Table 18 | Python mesh, image, and scientific-computing packages | §17.3.3 |
| Table 19 | Python utility and auxiliary packages | §17.3.4 |
| Table 20 | Python HTTP, authentication, and CLI packages | §17.3.5 |
| Table 21 | Documentation-generation packages installed in this session | §17.3.6 |
| Table 22 | HuggingFace model artefacts: identifiers, on-disk sizes, and pipeline stages | §17.4 |
| Table 23 | Complete catalog of `.env` configuration variables | §17.5 |
...
The four hard requirements imposed by SCS's commercial deployment context are recorded verbatim in [PIVOT_BLUEPRINT.md §1](PIVOT_BLUEPRINT.md):

1. **Catalog of office equipment.** The system must recognise at minimum eleven categories of office furniture: office chair, desk, monitor, cabinet, bookshelf, lamp, desk lamp, keyboard, mouse, table, filing cabinet. These are the categories on which the existing CLIP classifier has been fine-tuned (13,752 images from Google Open Images V7, training documented in §4.5).
2. **IFC BIM compliance.** Output IFC files must use IFC4-conformant classes, with named entity types corresponding to the recognised category (e.g. an office chair becomes `IfcChair`, a desk becomes `IfcDesk`, a generic furnishing becomes `IfcFurnishingElement`). Geometry must be representable as `IfcTriangulatedFaceSet` for arbitrary meshes.
3. **xeokit visualisation with manual drag/drop.** The output meshes must load into the xeokit web viewer, support manual drag-and-drop placement within a room scene, and be reusable: the same chair recognised once must produce the same mesh when dropped into many rooms.
4. **Free, royalty-free, commercially-safe tooling.** No paid memberships. No revenue caps. No monthly-active-user caps. No geographic exclusions. No AGPL-class strong copyleft. No CC-BY-NC. The licence posture must be defensible to legal review.
...
**Cloud-API alternatives that were not adopted, and why.**
The 2026 landscape for image-to-3D contains several managed cloud services that, on the face of it, would be operationally simpler than a local pipeline: Tripo AI's cloud API, CSM 3D, Meshy.ai, Luma Labs' Genie, and various managed-inference offerings hosted by Replicate or RunPod. Each was considered against SCS's requirements and excluded for the following reasons:

1. **Per-image charges.** Cloud services bill per generation. At SCS-relevant throughput this becomes a non-trivial recurring expense and a per-customer variable cost that the local pipeline does not have.
2. **Customer-image transmission.** A managed-cloud API requires uploading the customer's office photograph to the vendor. For BIM workflows in regulated sectors (financial services, government, healthcare) this is operationally non-trivial; for some SCS clients it would be a contractual obstacle.
3. **Vendor lock-in.** A managed API's behaviour can change without notice (model upgrades, deprecations, pricing changes). A locally-deployed model under a permissive licence is durable on the timescales SCS needs.
4. **Licensing transparency.** A managed API's licence applies only to the input/output relationship; the model behind it may be under unknown terms. Local deployment forces explicit verification (the audit conducted in §6.4) and produces a defensible licence posture.
5. **Latency tail.** Cloud APIs add network round-trip and shared-tenant queueing latency. For interactive SCS use cases this is undesirable.
...
A parallel investigation across the `TripoSR-SAM2-Humphrey-Enhanced` branch attempted to improve TripoSR output quality by chaining SAM 2 segmentation, Humphrey smoothing, and Poisson surface refinement. Commit `6e3ae89` introduced the full chain; commit `345baf6` reverted it after evaluation showed that the smoothing and refinement steps removed geometric details that the downstream IFC export needed. The lesson is captured in [TripoSR_CHANGES_AND_LESSONS.md](TripoSR_CHANGES_AND_LESSONS.md): *post-hoc smoothing of TripoSR output trades local detail for global smoothness, which is the wrong trade for office furniture detail* (e.g. it rounds chair leg terminations and removes joint articulations). This investigation produced a durable lesson on the limits of post-processing-as-fix for a fundamentally limited generation pipeline, and contributed materially to the later pivot decision.

### 4.5 The CLIP Fine-Tuning Effort (`Dimitres.Iteration3`, May 2026)

The `Dimitres.Iteration3` branch saw the introduction of three Python scripts: [scripts/download_openimages.py](scripts/download_openimages.py) (S3 direct downloads of Google Open Images V7 — no AWS-CLI dependency, no API key required, CC-BY-4.0 data), [scripts/train_clip_office.py](scripts/train_clip_office.py) (a CLIP ViT-B/32 fine-tune on 11 office furniture categories using either a linear probe or LoRA adapter approach), and [scripts/evaluate_clip.py](scripts/evaluate_clip.py) (a side-by-side zero-shot vs fine-tuned comparison). The training corpus was 13,752 images with the per-category counts:
...
The investigation that culminated in [PIVOT_BLUEPRINT.md](PIVOT_BLUEPRINT.md) made three observations that, taken together, motivated the architectural pivot:

1. The four failure modes of single-view generative reconstruction (§3.2) are not patchable by post-processing (per §4.4) and are not fixable by switching generative models (per §3.3). The class of approach is structurally inadequate to the SCS use case.
2. Industrial BIM practice converges on the retrieval framing: Revit families, BIMobject, RevitCity, Sketchfab CC0 catalogs. The industry does not generate office furniture from photographs; it selects from libraries. The SCS-distinctive value is the automation of the *selection*.
3. The Amazon Berkeley Objects dataset (7,953 artist-authored meshes, CC-BY-4.0) [3, 62] provides a sufficient library for the eleven SCS categories, and DINOv2 features [13] provide a sufficient embedding for fine-grained retrieval, with the FAISS library [6] handling the nearest-neighbour index at scale.
...
|---|---|---|---|
| `retrieval-pivot-blueprint` | `c78f3ac` | 2026-05-21 | Salvaged Sprint 3/4/5/8 work into retrieval line |
| `Dimitres.Iteration3` | `52c1062` | 2026-05-06 | Open Images downloader S3 URLs |
| `Original-TripoSR` | `93c2a94` | 2026-05-05 | All eight sprints |
| `TripoSR-SAM2-Humphrey-Enhanced` | `18c9ce8` | 2026-04-30 | SAM2 + Humphrey + k-means (reverted experiment) |
| `all-documentation` | `4394266` | 2026-05-05 | Consolidated documentation folder |
| `main` | `3c007eb` | 2026-05-05 | Real IFC4 export |
| `phase-2-sprints` | `2d068d8` | 2026-05-05 | PHASE2_SPRINTS.md docs |
| `phase-1-infrastructure` | `dd12931` | 2026-04-21 | Phase 8 testing & docs |
| `File-4-21-2026` | `c71b00e` | 2026-04-15 | Merge of main |
...
The backend is structured as four sub-modules under [backend/](backend/):
...
1. Frontend collects scene objects with attached `glbPath` references.
2. POST to `/api/export/ifc` with a JSON body listing objects + transforms.
3. Backend invokes `saveIFC.py` via the Python bridge, passing the JSON as a single argument (avoiding shell-argument-length limits).
4. `saveIFC.py` loads each referenced GLB with `trimesh`, decimates to ≤ 8000 faces (Mesh complexity beyond this point yields IFC files that overload Revit's IFC importer), constructs an `IfcTriangulatedFaceSet` per object, and writes an IFC4 file with the proper Project → Site → Building → Storey → Furniture hierarchy.
5. Backend streams the IFC bytes to the client as a download.
...
1. Listed on HuggingFace with a stated licence on the model card.
2. Licence is verifiable from primary sources (the HuggingFace model card, the linked LICENSE file, or the originating GitHub repository's LICENSE).
3. Licence either grants commercial use unconditionally (Apache-2.0, MIT, BSD-3, public-domain-equivalent) or grants it under conditions that SCS can meet without further negotiation (the SAM License, the Stability Community License at less than US$1M annual revenue).
4. Inference VRAM either fits the 8 GB primary box at native precision or can be made to fit by quantisation or CPU-offload at acceptable latency.
5. The model addresses one or more of: detection of office furniture, segmentation, depth estimation, metric scale, single-image-to-3D reconstruction, image embedding for retrieval, material extraction.

The exclusion criteria are:

1. AGPL-3.0, GPL-3.0, or any strong-copyleft licence.
2. CC-BY-NC-4.0 or any non-commercial Creative Commons variant.
3. Research-only licences (`apple-amlr`, OpenRAIL non-commercial, any "research" tag without an explicit commercial grant).
4. Model cards that disclaim commercial deployment in the body text even when the file licence is permissive (this is the OpenAI CLIP situation: file licence MIT, but model card text *"Any deployed use case … is currently out of scope"*).
...
Table 6.2 below summarises the ten cross-pipeline candidates. The full per-model analysis is provided in [MODEL_SURVEY_SCS.md](MODEL_SURVEY_SCS.md); key entries are reproduced here.

| # | Model | HF repo | License (verbatim) | Native VRAM | Output | SCS Role |
|---|---|---|---|---|---|---|
| 1 | DINOv2-Large | `facebook/dinov2-large` | `apache-2.0` | ~1.5 GB | 1024-d image embedding | Primary retrieval embedding |
| 2 | SigLIP 2 so400m | `google/siglip2-so400m-patch14-384` | `apache-2.0` | ~4 GB | Image + text embedding | Alternate retrieval / zero-shot classification |
| 3 | SAM 2.1 hiera-large | `facebook/sam2.1-hiera-large` | `Apache-2.0` | ~3 GB | Segmentation mask | Primary segmentation |
| 4 | Grounding DINO base | `IDEA-Research/grounding-dino-base` | `apache-2.0` | ~3 GB | Open-vocab bounding boxes | Primary detection |
| 5 | Depth Anything V2 Small | `depth-anything/Depth-Anything-V2-Small-hf` | `apache-2.0` | ~0.5 GB | Relative depth map | Metric scale |
| 6 | SAM 3D Objects | `facebook/sam-3d-objects` | SAM License (`other`) | ~24 GB est. | Posed 3D mesh + texture | Fallback for missing-catalog items (24 GB box) |
| 7 | SAM 3 | `facebook/sam3` | SAM License | ~4 GB | Promptable segmentation | Segmentation upgrade |
| 8 | Stable Fast 3D | `stabilityai/stable-fast-3d` | Stability Community License | ~7 GB | Mesh + PBR (albedo, roughness, metalness) | PBR-emitting fallback (revenue-conditional) |
| 9 | TRELLIS-image-large | `microsoft/TRELLIS-image-large` | `MIT` | ~16 GB | Posed 3D mesh (SLAT diffusion) | Generative fallback on 24 GB box |
| 10 | TripoSR | `stabilityai/TripoSR` | `MIT` | ~4 GB | Mesh (LRM) | Last-resort fallback |
...
Table 6.3 below summarises ten models specifically suited to detecting, classifying, or segmenting office furniture. The full analysis is in [OFFICE_FURNITURE_DETECTION_BENCHMARK.md](OFFICE_FURNITURE_DETECTION_BENCHMARK.md).

| # | Model | HF repo | License | Vocab | Office furniture coverage |
|---|---|---|---|---|---|
| 1 | DETR ResNet-50 | `facebook/detr-resnet-50` | `apache-2.0` | Closed (COCO 80) | chair, table, couch, tv, laptop, keyboard, mouse, book |
| 2 | DETR ResNet-101 | `facebook/detr-resnet-101` | `apache-2.0` | Closed (COCO 80) | Same as DETR-50 with higher mAP |
| 3 | RT-DETR R101 VD | `PekingU/rtdetr_r101vd_coco_o365` | `apache-2.0` | Closed (COCO + Objects365 pretrain) | Trained on Objects365 → much broader furniture exposure |
| 4 | DETA Swin-Large | `jozhang97/deta-swin-large` | `apache-2.0` (GitHub LICENSE) | Closed (COCO 80) | Highest COCO mAP in the closed-vocab set (50.2) |
| 5 | Grounding DINO base | `IDEA-Research/grounding-dino-base` | `apache-2.0` | Open-vocab | Any furniture nameable in text — covers all 11 SCS categories directly |
| 6 | OWLv2 large ensemble | `google/owlv2-large-patch14-ensemble` | `apache-2.0` | Open-vocab | Any — ensemble for higher accuracy |
| 7 | Florence-2 large | `microsoft/Florence-2-large` | `MIT` | Multi-task (detection + caption + OCR) | Detection via `<OD>` prompt; caption disambiguates variants |
| 8 | OneFormer ADE20K Swin-L | `shi-labs/oneformer_ade20k_swin_large` | `MIT` | Closed (ADE20K 150) | chair, armchair, swivel chair, desk, sofa, cabinet, bookcase, shelf, lamp, computer, screen, monitor, fan |
| 9 | SigLIP 2 so400m | `google/siglip2-so400m-patch14-384` | `apache-2.0` | Zero-shot classification | Classify any text-described category on box crops |
| 10 | DINOv2-Large | `facebook/dinov2-large` | `apache-2.0` | Retrieval | Embed crops for fine-grained library matching |
...
1. **`yolov8n-seg.pt` at repo root** — AGPL-3.0 binary in the source tree. Status: present. Recommended action: delete and replace usage with SAM 2.1.
2. **Depth Anything V2 upgrade pathway** — code currently calls the Apache-2.0 *Small* variant; an engineer upgrading to the *Base* or *Large* variant for better depth quality would silently void commercial use. Status: latent. Recommended action: pin the model name in code and document the licence reason in a comment.
3. **Hunyuan3D path in salvage candidates** — [PROJECT_HISTORY.md](PROJECT_HISTORY.md) lists `run_hunyuan3d.py` from `Original-TripoSR` as a candidate for future salvage. Status: not in the present branch. Recommended action: revise the salvage roadmap to mark Hunyuan3D as permanently excluded on licence grounds.
...
1. `git clone https://github.com/Dimitres-Kisimov/3DpicToIFCModeling.git` into `c:\Users\dinos\Downloads\`.
2. `git checkout retrieval-pivot-blueprint` — at HEAD commit `c78f3ac`.
3. Read of the five canonical project documents: README.md, PROJECT_HISTORY.md, WORK_CHECKPOINT.md, PIVOT_BLUEPRINT.md, FULL_DOCUMENTATION.md.
4. `npm install` via `npm.cmd` (working around the Windows PowerShell ExecutionPolicy that blocks the default `npm.ps1`). 242 packages installed. 12 advisory vulnerabilities reported by `npm audit` (9 moderate, 1 high, 2 critical) — typical for an older lockfile and deferred to a security-cleanup sprint.
5. `python -m pip install --upgrade pip` followed by `python -m pip install torch torchvision --index-url https://download.pytorch.org/whl/cu126`. Installed PyTorch 2.12.0+cu126 plus dependencies. Total download approximately 2.6 GB for the torch wheel. The Python interpreter invoked is the system-installed CPython 3.13.3 at `C:\Users\dinos\AppData\Local\Programs\Python\Python313\python.exe`.
6. `python -m pip install transformers ultralytics "rembg[cpu]" trimesh scikit-image scipy pillow numpy ifcopenshell huggingface_hub`. Installed transformers 5.10.2, ultralytics 8.4.60, rembg 2.0.76, trimesh 4.12.2, scikit-image 0.26.0, scipy 1.17.1, ifcopenshell 0.8.5, huggingface_hub 1.18.0, plus their dependencies.
7. Creation of `.env` from `.env.example` with the substantive deviation that `PYTHON_PATH` is set to the absolute path of the Python 3.13 executable rather than the plain `python`. This deviation is necessary because on the SCS workstation `python` in PATH resolves to the Microsoft Store stub launcher, not to Python 3.13.
8. Verification: `python -c "import torch; print(torch.cuda.is_available(), torch.cuda.get_device_name(0))"` returned `True NVIDIA GeForce RTX 4070 Laptop GPU`.
9. Server start via `node backend/server.js` (not `npm start`, again because of the PowerShell ExecutionPolicy issue with `.ps1` shims). Server initialised on http://localhost:3000.
10. Health verification: `GET /api/health` returned `{success: true, ...}`; `GET /api/debug/health` reported Python 3.13.3, NumPy 2.4.4, PyTorch 2.12.0+cu126, CUDA True.
...
|---|---|---|
| **CPU** | Multi-core x86-64 processor (8th-gen-or-later Intel / Ryzen-class) | Hosts the Node.js server, the Python subprocess bridge, image-decode (libjpeg-turbo, libpng), `trimesh` mesh ops, IfcOpenShell IFC writing |
| **System RAM** | 64 GB DDR | Holds the Node.js heap (~150 MB), Python interpreter heap (~500 MB per loaded model not counting tensors), CPU-offload tensor overflow when GPU VRAM exhausted, image buffers, `trimesh` working set during decimation, the FAISS retrieval index in production (~1.5 GB for an ABO subset of ~1500 meshes pre-embedded with DINOv2-Large) |
| **GPU model** | NVIDIA GeForce RTX 4070 Laptop GPU | Forward passes of all AI models in the pipeline |
| **GPU architecture** | Ada Lovelace (compute capability 8.9) | Native support for FP16, BF16, INT8, FP8 quantisation; fourth-generation tensor cores; second-generation RT cores (not used by our pipeline) |
| **VRAM** | 8 GB GDDR6 | Holds model weights at inference precision, activation tensors during forward pass, input image batch buffers |
| **VRAM bandwidth** | ~256 GB/s (estimated for the 4070 Laptop variant) | Throughput bottleneck for large-tensor operations; matters most for the segmentation models (SAM 2.1 hiera-large) |
| **GPU driver** | NVIDIA driver 572.83 | Reports CUDA 12.8 capability — forward-compatible with cu126 PyTorch wheels |
| **CUDA toolkit** | Implied 12.6 via PyTorch cu126 wheels | The wheels bundle their own CUDA runtime libraries; no system CUDA install required |
| **Storage** | NVMe SSD (assumed for PyTorch first-run download performance) | Hosts the HuggingFace cache, ~20 GB after all recommended models cached |
| **OS** | Windows 11 Pro 64-bit | Imposes the PowerShell ExecutionPolicy and Microsoft-Store Python-stub friction points documented in §9.5 |
| **PyTorch** | 2.12.0+cu126 | Inference framework |
| **Python** | 3.13.3 (CPython, system install) | Interpreter |
| **Node.js** | 24.15.0 | Backend runtime |
...
| Pipeline stage | Binding constraint | Headroom on tested workstation |
|---|---|---|
| Photograph upload + decode | System RAM (image buffer + multipart parse) | Comfortable; 50 MB max upload at 64 GB RAM |
| Grounding DINO detection | GPU VRAM for model weights + activations | ~5 GB headroom after model + image — comfortable |
| SAM 2.1 segmentation | GPU VRAM + VRAM bandwidth (mask resolution) | ~5 GB headroom — comfortable |
| Object crop preparation | CPU + system RAM | Negligible |
| DINOv2 embedding | GPU VRAM (model weights only — single forward) | ~6.5 GB headroom — very comfortable |
| FAISS retrieval | System RAM (index) + CPU (cosine similarity) | Sub-50 ms on 1500-vector index — negligible |
| Depth Anything V2 Small | GPU VRAM | ~7.5 GB headroom — trivial |
| Mesh decimation (trimesh) | CPU + system RAM | Bounded by per-mesh face count after decimation cap of 8000 — negligible at this size |
| IfcOpenShell IFC writing | CPU + system RAM | Sub-second for the present scale |
| GLB → XKT conversion (xeokit-convert) | CPU (single-threaded Node script) | Seconds per file at the present scale |
| **SAM 3D Objects fallback (24 GB box only)** | **GPU VRAM** | **0 GB headroom on tested workstation — this is the reason for the secondary-box recommendation** |
| Stable Fast 3D fallback | GPU VRAM | ~1 GB headroom — fits but with no margin for batch >1 |
...
|---|---|---|
| CPU | 8-core modern x86-64 (Intel Core i7-13700H or Ryzen 7 7840HS or better) | Adequate for Node + Python without becoming a bottleneck against GPU |
| RAM | 32 GB minimum, 64 GB preferred | 64 GB needed for headroom on CPU-offload of larger generative models |
| GPU | RTX 4070 Laptop / 4070 / 4080 with ≥ 8 GB VRAM | Matches the tested baseline |
| Storage | 1 TB NVMe SSD | HuggingFace cache + datasets + working files |
| OS | Windows 11 Pro or Ubuntu 22.04 LTS | Either works; the workflow has been tested on Windows |

**Scenario B: Single-machine deployment with on-demand high-fidelity fallback** (e.g. an SCS developer demo workstation)

| Component | Recommended | Rationale |
|---|---|---|
| CPU | 12-core (Intel i9 / AMD Ryzen 9) | Less constrained for batch IFC export |
| RAM | 64 GB | Required for CPU offload of generative fallback when invoked |
| GPU | RTX 4090 24 GB (desktop) | Enables synchronous SAM 3D Objects on the same workstation |
| Storage | 2 TB NVMe SSD | Larger headroom; faster cache hit on warm models |
| OS | Ubuntu 22.04 LTS | Better PyTorch / CUDA ergonomics for batch work |

**Scenario C: Multi-user batch back-office for SCS at scale**

| Component | Recommended | Rationale |
|---|---|---|
| CPU | 32-core server (Xeon / EPYC) | Parallel Python subprocess workers |
| RAM | 256 GB ECC | Multiple FAISS indices, multiple model copies, large batch buffers |
| GPU | RTX 6000 Ada 48 GB or A100 40 GB | Concurrent generative inference on multiple customer images |
| Storage | 4 TB NVMe RAID + 16 TB HDD archive | Hot working set on NVMe; cold archived photos / IFCs on HDD |
| Networking | 10 GbE LAN | Photo upload throughput when batching from multiple sources |
| OS | Ubuntu 22.04 LTS server | Standard cloud-native server posture |
...
| Stage | Estimated wall-clock latency | Notes |
|---|---|---|
| Model cold-load (per model, first request after server start) | 6–10 s | Disk read + tensor unpack + CUDA kernel JIT |
| Grounding DINO detection on 800×800 image | 200–400 ms | Open-vocab detection over the 11-prompt query |
| SAM 2.1 mask of one box | 150–300 ms per mask | Multiple masks add roughly linearly |
| DINOv2 embedding of a 224×224 crop | 30–60 ms | Cheap |
| FAISS NN lookup on 1500-vector index | 1–5 ms | Negligible |
| Depth Anything V2 Small on 800×800 image | 50–100 ms | Cheap |
| trimesh decimation of a ~50k-face library mesh to 8k | 200–500 ms | CPU-bound |
| IfcOpenShell IFC4 write of one furniture entity | 50–100 ms | I/O bound on small files |
| xeokit-convert IFC → XKT | 1–3 s | Single-threaded Node process |
| **SAM 3D Objects on 24 GB box (when invoked)** | **30–60 s** | The reason this is the async fallback, not the synchronous primary |
...
| Sprint | Title | Owner | Acceptance criterion |
|---|---|---|---|
| S1 | Replace YOLOv8 with SAM 2.1 in `inference_base.py`; delete `yolov8n-seg.pt` from repo root | Engineer 1 | All existing segmentation calls succeed; AGPL binary removed from `git ls-files` |
| S2 | Cherry-pick CLIP fine-tuned checkpoint from `Dimitres.Iteration3` | Engineer 1 | `models/clip_office/best_model.pt` present on `retrieval-pivot-blueprint` |
| S3 | Curate Amazon Berkeley Objects subset for the 11 SCS categories; normalise, count | Engineer 2 | `data/furniture_library/manifest.json` with ≥ 500 meshes, all watertight |
| S4 | Pre-compute DINOv2 embeddings for the ABO subset; build FAISS index | Engineer 1 | `data/furniture_library/dinov2_index.faiss` built; nearest-neighbour query returns sub-50ms on 8 GB GPU |
| S5 | Implement retrieval call in `inference_base.py:classify_object_retrieval`; wire confidence-threshold gate | Engineer 1 | End-to-end retrieval test on 30 SCS office photos achieves ≥ 70% top-1 accuracy on the 11 categories |
| S6 | Curate SCS labelled test set (30+ photos, manual annotations) | Engineer 2 | `data/scs_test_set/` complete; checked into repo |
| S7 | Extend benchmark harness to compute mAP and recall@k against ground truth | Engineer 1 | `scripts/test_furniture_detection.py --eval-coco data/scs_test_set/annotations.json` produces per-model report |
| S8 | Wire SAM 3D Objects async fallback on 24 GB workstation; async queue implementation | Engineer 1 | Sub-threshold retrieval requests are correctly forwarded and returned within ≤ 90 s |
| S9 | Replace generative model picker in frontend with retrieval-first UX; demote generative models to "advanced" submenu | Engineer 2 | Default user flow uses retrieval; generative is opt-in |
| S10 | End-to-end demo: phone photo → uploaded → recognised → IFC → loaded into xeokit → drag into room | Both | Live demo on real SCS office photo |
...
|---|---|---|---|
| ABO subset insufficient coverage for SCS categories | Medium | Medium | Supplement with curated Sketchfab CC-BY meshes (S3 extension) |
| SAM 3D Objects requires HuggingFace agreement at every workstation | High | Low | Pre-download weights to shared storage; document onboarding step |
| 24 GB workstation unavailable | Medium | High | Fall back to Path B; accept long-tail degradation |
| Stability Community License revenue threshold crossed during life of project | Low | Medium | Drop Stable Fast 3D from architecture; substitute SAM 3D Objects |
| Future Depth Anything model release supersedes V2 Small | High | Low | Pin the model in code; review licence before any upgrade |
| Detection on multi-object photographs underperforms in clutter | Medium | Medium | Ensemble Grounding DINO with OWLv2; add a manual-correction UX |

### 11.4 Decisions Requested of SCS Stakeholders

Three decisions require SCS-level sign-off before further engineering:

1. **Path A or Path B?** Confirm whether the 24 GB workstation is reliably available for the lifetime of the project, or whether Path B (no generative fallback) is preferred.
2. **Stable Fast 3D adoption?** SCS finance to confirm revenue position relative to the US$1M Stability Community License threshold. If under, Stable Fast 3D can be substituted for SAM 3D Objects with marginally better PBR fidelity at the cost of revenue-cap risk.
3. **Long-tail handling policy?** [PIVOT_BLUEPRINT.md §5](PIVOT_BLUEPRINT.md#L196) lists three options (return best library match, generative fallback, refuse and prompt). Pick one before S5 implementation.
...
[50] PIVOT_BLUEPRINT.md (commit 10d1d9f, 21 May 2026). The architectural decision record establishing the retrieval pivot. In-project file: [PIVOT_BLUEPRINT.md](PIVOT_BLUEPRINT.md).

[51] PROJECT_HISTORY.md (commit 013985c, 21 May 2026). The branch-level engineering history relied upon throughout §4. In-project file: [PROJECT_HISTORY.md](PROJECT_HISTORY.md).

[52] README.md. Project README. In-project file: [README.md](README.md).

[53] WORK_CHECKPOINT.md. Engineering progress notes. In-project file: [WORK_CHECKPOINT.md](WORK_CHECKPOINT.md).

[54] FULL_DOCUMENTATION.md. Consolidated documentation index. In-project file: [FULL_DOCUMENTATION.md](FULL_DOCUMENTATION.md).

[55] TripoSR_CHANGES_AND_LESSONS.md. Engineering retrospective on the TripoSR + SAM2 + Humphrey reverted experiment. In-project file: [TripoSR_CHANGES_AND_LESSONS.md](TripoSR_CHANGES_AND_LESSONS.md).

[56] DEVELOPMENT_ROADMAP_PHASE2.md. Sprint roadmap predating the pivot. In-project file: [DEVELOPMENT_ROADMAP_PHASE2.md](DEVELOPMENT_ROADMAP_PHASE2.md).

[57] TEAM_ROADMAP.md. Team-level milestone plan. In-project file: [TEAM_ROADMAP.md](TEAM_ROADMAP.md).

[58] MODEL_SURVEY_SCS.md (this project, 6 June 2026). The cross-pipeline model survey supporting §6.2. In-project file: [MODEL_SURVEY_SCS.md](MODEL_SURVEY_SCS.md).

[59] OFFICE_FURNITURE_DETECTION_BENCHMARK.md (this project, 6 June 2026). The detection-specific model survey supporting §6.3. In-project file: [OFFICE_FURNITURE_DETECTION_BENCHMARK.md](OFFICE_FURNITURE_DETECTION_BENCHMARK.md).

[60] SESSION_REPORT.md (this project, 6 June 2026). The setup and research session log. In-project file: [SESSION_REPORT.md](SESSION_REPORT.md).
...
| Model | HF repo | License string (verbatim) | Verification source |
|---|---|---|---|
| DINOv2-Large | `facebook/dinov2-large` | `apache-2.0` | HF model card |
| SigLIP 2 so400m | `google/siglip2-so400m-patch14-384` | `apache-2.0` | HF model card |
| SAM 2.1 hiera-large | `facebook/sam2.1-hiera-large` | `Apache-2.0` | HF model card |
| Grounding DINO base | `IDEA-Research/grounding-dino-base` | `apache-2.0` | HF model card |
| Depth Anything V2 Small | `depth-anything/Depth-Anything-V2-Small-hf` | `apache-2.0` | HF model card |
| Depth Anything V2 Base | `depth-anything/Depth-Anything-V2-Base-hf` | `cc-by-nc-4.0` | HF model card |
| Depth Anything V2 Large | `depth-anything/Depth-Anything-V2-Large-hf` | `cc-by-nc-4.0` | HF model card (extrapolated from Base) |
| SAM 3D Objects | `facebook/sam-3d-objects` | `other` / SAM License | HF model card + linked LICENSE |
| SAM 3 | `facebook/sam3` | `other` / SAM License | HF LICENSE file |
| Stable Fast 3D | `stabilityai/stable-fast-3d` | Stability Community License (≤US$1M) | HF model card |
| TRELLIS-image-large | `microsoft/TRELLIS-image-large` | `MIT` | HF model card |
| TripoSR | `stabilityai/TripoSR` | `MIT` | HF model card |
| Hunyuan3D-2 | `tencent/Hunyuan3D-2` | Tencent Hunyuan 3D 2.0 Community License | HF LICENSE file |
| DETR ResNet-50 | `facebook/detr-resnet-50` | `apache-2.0` | HF model card |
| DETR ResNet-101 | `facebook/detr-resnet-101` | `apache-2.0` | HF model card |
| RT-DETR R101 VD | `PekingU/rtdetr_r101vd_coco_o365` | `apache-2.0` | HF model card |
| DETA Swin-Large | `jozhang97/deta-swin-large` | `apache-2.0` | GitHub LICENSE |
| OWLv2 large ensemble | `google/owlv2-large-patch14-ensemble` | `apache-2.0` | HF model card |
| Florence-2 large | `microsoft/Florence-2-large` | `MIT` | HF model card |
| OneFormer ADE20K Swin-L | `shi-labs/oneformer_ade20k_swin_large` | `MIT` | HF model card |
| YOLOv8 (Ultralytics) | n/a — file in repo | `AGPL-3.0` | Ultralytics public docs |
| Apple DepthPro | `apple/DepthPro` | `apple-amlr` | HF model card |
| OpenAI CLIP ViT-L | `openai/clip-vit-large-patch14` | MIT file + model card disclaimer | HF model card |
| Marigold depth | `prs-eth/marigold-depth-lcm-v1-0` | `cc-by-sa-4.0` | HF model card |

---

## 15. Appendix B: Test Harness Reference

The detection benchmark harness is at [scripts/test_furniture_detection.py](scripts/test_furniture_detection.py). Its full usage is documented in the module docstring; the salient commands are reproduced here.
...
Outputs:
- `outputs/detection_benchmark.csv` — per-model summary with columns `key, model, ok, error, n_detections, top5, latency_ms, peak_vram_mb`.
- `outputs/detection_overlays/<model_key>.jpg` — annotated input image per model showing detected boxes and labels.
...
For ease of cross-reference, the relationships among the nine branches are reproduced from [PROJECT_HISTORY.md](PROJECT_HISTORY.md) §2:
...
| Component | Installed version | Role | Install method |
|---|---|---|---|
| Windows | 11 Pro 64-bit | Host OS | Pre-installed |
| Node.js | 24.15.0 | Backend runtime | nodejs.org installer; located at `C:\Program Files\nodejs\node.exe` |
| npm | 11.12.1 | Node package manager | Bundled with Node.js |
| Python | 3.13.3 (CPython) | AI runtime | python.org installer; located at `C:\Users\dinos\AppData\Local\Programs\Python\Python313\python.exe` |
| pip | 26.1.2 | Python package manager | Upgraded during setup |
| Git | 2.54.0 (windows.1) | Source control | git-scm.com installer |
| NVIDIA driver | 572.83 | GPU access | NVIDIA installer; reports CUDA 12.8 capability |
| PowerShell | 5.1 (Windows PowerShell) | Shell for setup commands | Bundled with Windows |
...
|---|---|---|
| @xeokit/xeokit-convert | ^1.1.27 | IFC → XKT converter CLI for browser delivery |
| @xeokit/xeokit-sdk | ^2.6.108 | Browser-side 3D viewer (WebGL); MIT licence |
| axios | ^1.4.0 | HTTP client for in-server fetches |
| cors | ^2.8.5 | CORS middleware |
| dotenv | ^16.0.3 | `.env` file loader |
| express | ^4.18.2 | HTTP server framework |
| multer | ^1.4.5-lts.1 | Multipart upload parser |
| uuid | ^9.0.0 | UUID generator for upload identifiers |
| nodemon | ^3.0.1 (devDependency) | Dev-mode auto-restart |
...
| Package | Installed version | Role |
|---|---|---|
| torch | 2.12.0+cu126 | PyTorch 2.12 with bundled CUDA 12.6 runtime |
| torchvision | 0.27.0+cu126 | Image transforms and torch model zoo |
| filelock | 3.29.0 | torch's file-lock dependency |
| sympy | 1.14.0 | torch's symbolic-math dependency |
| networkx | 3.6.1 | torch dependency for computation graphs |
| jinja2 | 3.1.6 | torch dependency for templating |
| MarkupSafe | 3.0.3 | jinja2 dependency |
| mpmath | 1.3.0 | sympy dependency |
| typing-extensions | 4.15.0 | torch dependency |
| setuptools | 70.2.0 | torch pinned-build dependency |
| fsspec | 2026.4.0 | torch dependency for storage abstractions |

#### 17.3.2 ML and HuggingFace Stack

| Package | Installed version | Role |
|---|---|---|
| transformers | 5.10.2 | HuggingFace inference library; loads every AI model in the recommended stack |
| huggingface_hub | 1.18.0 | Weight download and authentication |
| hf-xet | 1.5.0 | Newer HuggingFace transfer protocol |
| tokenizers | 0.22.2 | Tokeniser for the text encoders of CLIP, SigLIP, Grounding DINO |
| safetensors | 0.7.0 | Safer weight-file format |
| ultralytics | 8.4.60 | YOLO inference (vendored for compatibility; **AGPL — to be removed**, see §6.5) |
| ultralytics-thop | 2.0.20 | FLOPs counter for YOLO |
| rembg | 2.0.76 | Background removal via U²-Net |
| pymatting | 1.1.15 | Alpha-matting library used by rembg |

#### 17.3.3 Mesh, Image, and Scientific Computing

| Package | Installed version | Role |
|---|---|---|
| numpy | 2.4.4 | Numerical arrays; foundation of every downstream library |
| scipy | 1.17.1 | Scientific computing (spatial transforms, linear algebra) |
| scikit-image | 0.26.0 | Image processing (used for marching-cubes patch in TripoSR) |
| trimesh | 4.12.2 | Mesh I/O, decimation, transforms |
| shapely | 2.1.2 | 2D geometry primitives (used by spatial layout solver) |
| pillow | 12.2.0 | Image I/O |
| imageio | 2.37.3 | Image format support |
| tifffile | 2026.6.1 | TIFF support |
| opencv-python | 4.13.0.92 | Image processing fallbacks |
| matplotlib | 3.10.9 | Diagnostic visualisation |
| ifcopenshell | 0.8.5 | IFC4 reading and writing |
| onnxruntime | 1.26.0 | ONNX inference (used by rembg) |

#### 17.3.4 Utilities and Auxiliary

| Package | Installed version | Role |
|---|---|---|
| polars | 1.41.2 | Fast DataFrame (alternative to pandas) |
| polars-runtime-32 | 1.41.2 | Polars 32-bit runtime |
| numba | 0.65.1 | JIT for performance-critical Python loops |
| llvmlite | 0.47.0 | Numba dependency |
| pyyaml | 6.0.3 | YAML config parsing |
| protobuf | 7.35.0 | ONNX protobuf encoding |
| flatbuffers | 25.12.19 | onnxruntime dependency |
| psutil | 7.2.2 | System resource monitoring |
| platformdirs | 4.10.0 | Cross-platform path resolution |
| referencing | 0.37.0 | JSON Schema dependency |
| jsonschema | 4.26.0 | JSON validation |
| jsonschema-specifications | 2025.9.1 | JSON Schema specs |
| rpds-py | 2026.5.1 | jsonschema dependency |
| attrs | 26.1.0 | Class-decorator library |
| annotated-doc | 0.0.4 | Documentation annotation |
| pooch | 1.9.0 | Data file fetching (used by scikit-image) |
| lazy-loader | 0.5 | scikit-image lazy loader |
...
| isodate | 0.7.2 | ISO date format support |
| lark | 1.3.1 | Parser library |
| packaging | 26.2 | PEP 440 version handling |
...
| Package | Installed version | Role |
|---|---|---|
| requests | 2.34.2 | HTTP client (used by huggingface_hub) |
| urllib3 | 2.7.0 | requests dependency |
| certifi | 2026.5.20 | TLS root certificates |
| charset_normalizer | 3.4.7 | requests dependency |
| idna | 3.18 | requests dependency |
| httpx | 0.28.1 | Async HTTP client |
| httpcore | 1.0.9 | httpx dependency |
| h11 | 0.16.0 | HTTP/1.1 implementation |
| anyio | 4.13.0 | Async I/O abstraction |
| click | 8.4.1 | CLI framework (used by various tools) |
| colorama | 0.4.6 | Cross-platform terminal colours |
| typer | 0.25.1 | Modern click successor |
| shellingham | 1.5.4 | Shell detection for typer |
| rich | 15.0.0 | Pretty terminal output |
| pygments | 2.20.0 | Syntax highlighting in rich |
| markdown-it-py | 4.2.0 | Markdown parser used by rich |
...
| Package | Installed version | Role |
|---|---|---|
| pypandoc-binary | (latest) | Markdown → DOCX conversion (bundles pandoc) |
| python-docx | (latest) | Direct DOCX manipulation (fallback if pandoc unavailable) |
...
| Model | Repo identifier | On-disk size | Stage in pipeline |
|---|---|---|---|
| DINOv2-Large | `facebook/dinov2-large` | ~1.2 GB | Retrieval embedding |
| SAM 2.1 hiera-large | `facebook/sam2.1-hiera-large` | ~0.9 GB | Segmentation |
| Grounding DINO base | `IDEA-Research/grounding-dino-base` | ~0.7 GB | Open-vocab detection |
| Depth Anything V2 Small | `depth-anything/Depth-Anything-V2-Small-hf` | ~0.1 GB | Metric scale |
| SigLIP 2 so400m | `google/siglip2-so400m-patch14-384` | ~4.0 GB | Zero-shot classification (optional) |
| Florence-2 large | `microsoft/Florence-2-large` | ~1.6 GB | Multi-task (optional) |
| OneFormer ADE20K Swin-L | `shi-labs/oneformer_ade20k_swin_large` | ~0.9 GB | Segmentation alternative |
| TripoSR | `stabilityai/TripoSR` | ~1.3 GB | Last-resort generative fallback (already cached) |
| **SAM 3D Objects (24 GB box only)** | `facebook/sam-3d-objects` | ~10 GB (estimated) | Generative fallback |
...
| Variable | Present value | Role |
|---|---|---|
| `PORT` | `3000` | TCP port for the Express server |
| `HOST` | `localhost` | Bind address; `localhost` to refuse non-loopback access during development |
| `NODE_ENV` | `development` | Express mode; toggles stack-trace exposure on error responses |
| `MAX_FILE_SIZE` | `52428800` (50 MB) | Upload size limit accommodating phone photos |
| `TEMP_DIR` | `./temp` | Intermediate file working directory |
| `UPLOAD_DIR` | `./uploads` | Inbound image storage |
| `OUTPUT_DIR` | `./outputs` | Generated GLB / IFC / XKT storage |
| `PYTHON_PATH` | `C:/Users/dinos/AppData/Local/Programs/Python/Python313/python.exe` | Absolute Python executable; absolute path used to bypass the Microsoft-Store stub `python` in PATH |
| `PYTHON_SCRIPTS_DIR` | `./backend/python-scripts` | Location of AI subprocess entry-point scripts |
| `USE_GPU` | `true` | Toggles GPU vs CPU inference path |
| `CUDA_VISIBLE_DEVICES` | `0` | Restricts inference to GPU index 0 |
| `GPU_MAX_MEMORY_MB` | `8192` | Soft cap matching the 8 GB VRAM ceiling |
| `INSTANTMESH_MODEL_PATH` | (empty) | Reserved for local-cache override of the InstantMesh model (deprecated in retrieval pivot) |
| `STABLEFAST3D_MODEL_PATH` | (empty) | Reserved for local-cache override of Stable Fast 3D |
| `TRIPOSR_MODEL_PATH` | (empty) | Reserved for local-cache override of TripoSR |
| `IFC_OUTPUT_DIR` | `./outputs` | IFC writer's output directory |
...
- **Test image:** [`backend/triposr/examples/chair.png`](backend/triposr/examples/chair.png), 512×512 RGB, single side chair on transparent background (suitable as a clean-object smoke-test surrogate while a labelled SCS test set is being curated).
...
The harness in [`scripts/test_furniture_detection.py`](scripts/test_furniture_detection.py) was executed against the 10 candidate detection models from §6.3. The CSV is at [`outputs/sprint_test/detection_benchmark.csv`](outputs/sprint_test/detection_benchmark.csv) and annotated overlays in [`outputs/sprint_test/detection_overlays/`](outputs/sprint_test/detection_overlays/).

| Model | Result | Top detection | Latency (ms) | Peak VRAM (MB) |
|---|---|---|---|---|
| DETR ResNet-50 | ✅ ok | `chair` 0.98 | 7,612 | 344 |
| DETR ResNet-101 | ✅ ok | `chair` 1.00 | 67,355 | 425 |
| RT-DETR R101 VD | ✅ ok | `chair` 0.95 | 15,249 | 426 |
| DETA Swin-Large | ❌ broken | — | 443 | 9 |
| Grounding DINO base | ❌ broken | — | 36,742 | 1,736 |
| OWLv2 large ensemble | ❌ broken | — | 65,588 | 2,020 |
| Florence-2 large | ❌ broken | — | 2,640 | 9 |
| OneFormer ADE20K Swin-L | ✅ ok | `wall, chair, floor` | 35,883 | 1,437 |
| SigLIP 2 so400m | ⚠️ degenerate | scores all 0.0 | 177,656 | 4,415 |
| DINOv2-Large | ✅ ok | embedding produced (mean-|v| 1.16) | 41,280 | 1,213 |
...
| Model | Failure mode |
|---|---|
| **DETA Swin-Large** | The HF repo `jozhang97/deta-swin-large`'s `preprocessor_config.json` is missing the `image_processor_type` key required by `transformers` 5.10's `AutoImageProcessor`. Fix: explicitly load `DetaImageProcessor` (if present), or pin `transformers<5.0`. |
| **Grounding DINO base** | `GroundingDinoProcessor.post_process_grounded_object_detection()` keyword `box_threshold` was renamed in transformers 5.x. Fix: rename call-site argument to `threshold` or use the new `text_threshold` only. |
| **OWLv2 large** | `Owlv2Processor` no longer exposes `post_process_object_detection`. Fix: switch to the model's `post_process_grounded_object_detection` or use direct logit thresholding. |
| **Florence-2 large** | `Florence2LanguageConfig` is missing `forced_bos_token_id` attribute that the `transformers` 5.x generate() path queries. Fix: pin `trust_remote_code` to a Florence-2 commit predating the breakage, or set the attribute manually after load. |
| **SigLIP 2** | Model loaded but `sigmoid(logits_per_image)` collapsed to 0.0 across all 11 categories — input padding likely mis-configured. Fix: investigate `padding=max_length` vs. `padding=True`; verify the temperature parameter is read from the model card. |
...
- **Output:** [`outputs/sprint_test/chair_synth.ifc`](outputs/sprint_test/chair_synth.ifc) — **21,460 bytes**.
- **Schema:** IFC4 ✓
...
| Package | Required by | Action taken |
|---|---|---|
| `timm` | DETR / RT-DETR / DETA backbones | Installed on 2026-06-09; should be added to the documented dependency list |
| `einops` | TripoSR `tsr.system` | Installed on 2026-06-09 |
| `omegaconf`, `moderngl`, `xatlas` | TripoSR utilities (config parsing, GL render, UV unwrap) | Installed on 2026-06-09 |
...
All artefacts are in [`outputs/sprint_test/`](outputs/sprint_test/) and are reproducible from a clean clone after the `timm einops omegaconf moderngl xatlas` install.
...
| Tool | Vendor | Version | Used for |
|---|---|---|---|
| Claude Code | Anthropic | Opus 4.7 (CLI 2026-06-06 build) | Iterative drafting, structuring, table formatting, automated WebFetch licence verification against HuggingFace and linked LICENSE files, dependency-list capture from `pip list` and `npm ls` output, cross-document consistency checking, and conversion of the final Markdown source into DOCX form |
...
| **Identifier / Matrikel-Nr.** | _______________________________________________ |
